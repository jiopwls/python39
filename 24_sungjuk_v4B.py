...
def addSungJuk():
    ...
    # 성적처리 - tot, avg, grd를 매개변수로 넘겨서 총점, 평균, 학점을 계산한 후
    # 그 결과를 다시 tot, avg, grd에 저장
    # addSungJuk 함수에서 계산된 결과 확인
    tot, avg, grd = 0, 0, '가'
    computeSunkJuk(kor, eng, mat, tot, avg, grd)
...
def computeSunkJuk(kor, eng, mat, tot, avg, grd):
    tot = kor + eng + mat
    avg = tot / 3
    grd = '가'
    if avg >= 90:
