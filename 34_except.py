...
# 파이썬에서는 try-except 구문으로 예외처리를 할 수 있음

# error와 except 차이 비교
# 오류 : 프로그램 실행 중 오작동이나 비정상적 종료를 유발하게 하는 원인

# 컴파일 오류 : 주로 문법적 오류를 의미 - 개발자 조치 가능
# 논리 오류 : 실행 가능, 하지만 예상한 결과가 안나오는 오류 - 개발자 조치 가능
# 시스템 오류 : 메모리 초과, 서버접속 불가 등등 의 오류 - 조치 불가
# 실행 오류 : 프로그램 실행 중 발생하는 예외적인 오류
# ex) 어떤 값을 0으로 나누는 경우, 리스트의 인덱스를 잘못 호출하는 경우 등
# 잘못된 코드 사용으로 인한 오류
...
try:
    ...
    extype, exval, trackback = sys.exc_info() # 현재 발생한 예외 정보를 튜플로 반환
    print(extype.__name__) #예외 이름 (매직 키워드)
    print(trackback.tb_frame.f_code.co_filename) # 파일 명
